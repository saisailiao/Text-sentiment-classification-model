...
with open('./text_emotion/train_review_slim.csv', 'r', encoding="utf-8") as f:
    reviews = f.read()
with open('./text_emotion/train_label_slim.csv', 'r', encoding="utf-8") as f:
    labels = f.read()
with open('./text_emotion/test_review.csv', 'r', encoding="utf-8") as f:
    test = f.read()
...
all_text_test = ''.join([c for c in test if c not in punctuation])

# split by new lines and spaces
reviews_split = all_text.split('\n')
test_split = all_text_test.split('\n')
...
# Creating the model
embed_lookup = KeyedVectors.load_word2vec_format('./GoogleNews-vectors-negative300-SLIM.bin.gz', binary=True)
...
tokenized_test = tokenize_all_reviews(embed_lookup,test_split)
print(test_split[0])
...
class SentimentCNN(nn.Module):
    ...
    def __init__(self, embed_model, vocab_size, output_size, embedding_dim, hidden_dim, num_layers,
        ...
        # BiLSTM
        self.bilstm = nn.LSTM(embedding_dim, hidden_dim, num_layers=num_layers, dropout=drop_prob, bidirectional=False, bias=True)

        # 3. final, fully-connected layer for classification
        middle_size = len(kernel_sizes) * num_filters//2
    ...
    def forward(self, x):
        ...
        cnn_x = embeds.unsqueeze(1)

        # get output of each conv-pool layer
        conv_results = [self.conv_and_pool(cnn_x, conv) for conv in self.convs_1d]

        # concatenate results and add dropout
        cnn_x = torch.cat(conv_results, 1)
        cnn_x = self.dropout(cnn_x)

        # BiLSTM
        bilstm_x = embeds.view(len(x), embeds.size(1), -1)
        ...
        # linear
        cnn_out = self.fc(cnn_x)
        bilstm_out = self.fc2(bilstm_out)
        #print(bilstm_out.size())
        bilstm_out = bilstm_out.view(batch_size, -1)
        bilstm_out = bilstm_out[:,-1]  # get last batch of labels'''
        bilstm_out = bilstm_out.unsqueeze(0)
        bilstm_out = bilstm_out.t()
        #print(cnn_out)
        #print(bilstm_out)
        # output
        cnn_out = cnn_out*0.7
        bilstm_out = bilstm_out*0.3
        #print(cnn_out.size())
        #print(bilstm_out.size())

        logit = cnn_out+bilstm_out

        # sigmoid-activated --> a class score
        #print(self.sig(cnn_out))
        #print(self.sig(logit))
...
net = SentimentCNN(embed_lookup, vocab_size, output_size, embedding_dim,hidden_dim,num_layers,
...
for each in test_split:
    if(len(each)>0):
        predict_r = predict(embed_lookup, net, each,seq_length)
        result.append(predict_r)
        count+=1
        if count%100 == 0:
            print("Progress:{}".format(count))
            print("CurrentPrediction:{}".format(predict_r))
            if count == 25000:
                result_pd = pd.DataFrame(result, columns=['label'])
                print(result_pd.head())
                result_pd.to_csv('./text_emotion/test_result.csv', encoding="utf-8")



result_pd = pd.DataFrame(result,columns=['label'])
print(result_pd.head())
result_pd.to_csv('./text_emotion/test_result_2.csv',encoding="utf-8")
'''# set hyperparams
'''
seq_length = 200  # good to use the length that was trained on
